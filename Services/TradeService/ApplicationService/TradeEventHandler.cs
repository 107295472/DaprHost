...
namespace ApplicationService
{
    public class TradeEventHandler : IEventHandler
    {
        ...
        public TradeEventHandler(IOrderRepository repository, ILogisticsRepository logisticsRepository, ITradeLogRepository tradeLogRepository, IEventBus eventBus, IStateManager stateManager, IGoodsActorService goodsActorService)
        {
            ...
            return await new DefaultEventHandlerResponse().RunAsync(nameof(ExpireCancelOrder), input.GetDataJson(), async () =>
             {
                 ...
                 if (await cancelOrderService.Cancel(order))
                 {
                     ...
                     tradeLog.CreateTradeLog(TradeLogState.CancelOrder, eventData.OrderId, order.OrderNo, null, null,0, null);
